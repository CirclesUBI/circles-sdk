...
export class V1HubCalls {
    ...
    static trust(user: string): string {
        return this.encodeFunctionData('trust', [user, BigInt(100)]);
    }

    static untrust(user: string): string {
        return this.encodeFunctionData('trust', [user, BigInt(0)]);
    }
